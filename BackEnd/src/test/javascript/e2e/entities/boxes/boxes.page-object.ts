...
export class BoxesComponentsPage {
  createButton = element(by.id('jh-create-entity'));
  deleteButtons = element.all(by.css('jhi-boxes div table .btn-danger'));
  title = element.all(by.css('jhi-boxes div h2#page-heading span')).first();
...
export class BoxesUpdatePage {
  pageTitle = element(by.id('jhi-boxes-heading'));
  ...
  saveSelect = element(by.id('field_save'));
  ...
  async saveSelectLastOption(): Promise<void> {
    await this.saveSelect.all(by.tagName('option')).last().click();
  }

  async saveSelectOption(option: string): Promise<void> {
    await this.saveSelect.sendKeys(option);
  }

  getSaveSelect(): ElementFinder {
    return this.saveSelect;
  }

  async getSaveSelectedOption(): Promise<string> {
    return await this.saveSelect.element(by.css('option:checked')).getText();
  }
...
export class BoxesDeleteDialog {
  private dialogTitle = element(by.id('jhi-delete-boxes-heading'));
  private confirmButton = element(by.id('jhi-confirm-delete-boxes'));
